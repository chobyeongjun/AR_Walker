...
{
    _id = id;         // 이 컨트롤러가 담당할 관절의 고유 ID
    _data = exo_data; // 외골격 시스템 전체의 현재 상태 데이터를 담은 ExoData 객체의 포인터
    ...
    // PID 제어기 관련 초기값 설정
    _prev_input = 0; // 이전 입력 값 초기화 (미분 항 계산용)
    _prev_de_dt = 0; // 이전 오차 미분 값 초기화 (시간 단계가 불안정할 경우 사용)

    //  _controller_data와 _joint_data 포인터 설정 (가장 중요한 부분!)
...
//****************************************************
// _cf_mfac는 _Controller 클래스에 정의된 protected 멤버 함수로,
// Compact Form Model Free Adaptive Controller (CF-MFAC) 알고리즘
// Model Free Adaptive Controller"는 시스템의 정확한 수학적 모델(예: 모터의 정확한 운동 방정식)을 알지 못하더라도,
// 시스템의 입력과 출력 데이터만을 기반으로 스스로 학습하고 제어 성능을 개선하려는 제어 방식
...
float _Controller::_pid(float cmd, float measurement, float p_gain, float i_gain, float d_gain)
{
    ...
    {
        time_good = false; //  시간을 신뢰할 수 없다고 표시
    }
    ...
    float dt = (now - _prev_pid_time) * 1000000; // 이전 PID 호출 시간으로부터의 경과 시간 (마이크로초) 단위가 마이크로초 라서

    // Calculate the difference in the prescribed and measured torque
    float error_val = cmd - measurement; // 목표값(cmd) - 현재 측정값(measurement) = 오차
    ...
    if (i_gain != 0)
    {
        _pid_error_sum += error_val / LOOP_FREQ_HZ; // 오차를 루프 주파수로 나누어 누적 (시간에 대한 적분)
    }
    else
    {
        _pid_error_sum = 0;
    ...
    uint16_t exo_status = _data->get_status(); // _data (ExoData 객체)에서 현재 외골격 상태를 가져옴
    // 현재 상태가 '트라이얼 중', 'FSR 보정 중', 'FSR 미세 조정 중' 중 하나라도 일치하면 활성 트라이얼로 간주
    ...
    // 사용자가 일시 정지했거나 활성 트라이얼 상태가 아니라면
    ...
    if (time_good)
    {
        de_dt = -(measurement - _prev_input) * (1000.0f / LOOP_FREQ_HZ); // Convert to ms 마이크로 단위에서 ms로 변환
        _prev_de_dt = de_dt;                                             // 현재 계산된 미분 값을 다음 루프를 위해 저장
    }
    else // time_good이 false라면 (루프 시간이 너무 길었다면)
    {    // 미분 항은 시간에 매우 민감하므로, 시간이 불안정하면 계산하지 않고 0으로 설정
    ...
    return p + i + d; // 오차 보상에 필요한 PID 제어 신호를 반환
}
...
//  주로 외골격 개발자가 장치의 센서, 모터, 그리고 전체 시스템의 상태를 검증하고 보정하는 데 도움을 주는 자가 테스트 도구
// 모터의 방향 부호(+/-)가 올바른지, 토크 센서의 부호(+/-)가 원하는 방향과 일치하는지 확인하는 데 도움
// 오류 증폭(desired-measured 토크 간 오차 증폭)과 같은 안전 문제를 방지하기 위해 보정 절차를 수행하는 데 필수적인 도구
...
float CalibrManager::calc_motor_cmd()
{
    ...
    // '활성 트라이얼' 상태 정의: 트라이얼 중이거나, FSR 보정 중이거나, FSR 미세 조정 중인 경우
    bool active_trial = (exo_status == status_defs::messages::trial_on) ||
                        (exo_status == status_defs::messages::fsr_calibration) ||
                        (exo_status == status_defs::messages::fsr_refinement);

    // 2. 현재 상태 정보 시리얼 출력 (디버깅/사용자 피드백용)
    Serial.print("\nExo status: ");
    ...
    if (active_trial) // 시스템이 '활성 트라이얼' 상태라면 (보정 중 포함)
    {
        if (_joint_data->is_left)
        {
