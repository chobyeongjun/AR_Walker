...
InclineDetector::InclineDetector(float alpha, float threshold)
{
    ...
    _alpha = alpha;  // 외부에서 전달받은 필터 계수(alpha)와 임계값(threshold)을 멤버 변수에 저장합
    _threshold = threshold;

    // 초기 상태를 Level(평지)로 설정하고, 평균 압력(_avgPressure)과 평활화된 압력(_smooth_demeanedPressure) 변수를 초기화합니다. 
    // _avgPressure를 -1로 초기화하는 것은, 첫 번째 압력 데이터가 들어왔을 때 평균값을 계산하는 대신 그 값을 그대로 초기값으로 사용하려는 의도
    _avgPressure = -1;
    _state = Level;
