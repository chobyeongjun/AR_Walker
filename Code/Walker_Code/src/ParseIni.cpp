...
//The nano will get the info through SPI so doesn't need these functions.
    ...
    void ini_parser(char* filename, uint8_t* config_to_send)
    {
        ...
        pinMode(SD_SELECT, OUTPUT); // SD_SELECT pin을 OUTPUT모드로 설정하는 것, 지금부터 SD카드랑 대화할 것이라고 하는 코드
        digitalWrite(SD_SELECT, HIGH); //Disable SD card SD_SELECT 핀에 **HIGH 신호(끄기)**를 보냅니다. ** SPI 통신에서는 보통 LOW 신호를 보내야 장치가 활성화 **
        ...
        SPI.begin();  // SPI 통신을 시작하는 것, SD카드와 통신하기 위해 SPI를 초기화하는 코드
