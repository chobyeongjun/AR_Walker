...
typedef enum 
{                                               // UART 메시지의 안에서 각 정보의 위치에 사람이 이해하기 쉬운 이름을 붙이는 것
  COMMAND = 0,                                  // 0번은 Command, 1번은 Joint ID, 2번부터는 데이터 시작
  JOINT_ID = 1,
  DATA_START = 2
} UARTPackingIndex;
...
{
    static UARTHandler* instance = new UARTHandler(); // 최초로 get_instance()가 호출될 때만 인스턴스를 생성합니다. 2번째부턴 instance가 이미 생성되어 있어서 
    return instance;
}

void UARTHandler::UART_msg(uint8_t msg_id, uint8_t len, uint8_t joint_id, float *buffer) // 패킷(packet)으로 포장하여 전송하는 역할
{
    ...
    uint8_t _byte_data[_packed_len] = {0};             // 계산된 크기만큼의 **임시 저장 공간(버퍼)**인 _byte_data 배열을 생성
