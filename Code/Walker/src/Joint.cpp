...
HipJoint::HipJoint(config_defs::joint_id id, ExoData *exo_data)
    : _Joint(id, exo_data) // <-- Initializer list
      ,
      _zero_torque(id, exo_data), _franks_collins_hip(id, exo_data), _constant_torque(id, exo_data), _chirp(id, exo_data), _step(id, exo_data), _proportional_hip_moment(id, exo_data)
{
#ifdef JOINT_DEBUG
    logger::print(_is_left ? "Left " : "Right ");
...
ElbowJoint::ElbowJoint(config_defs::joint_id id, ExoData *exo_data)
    : _Joint(id, exo_data) // <-- Initializer list
      ,
      _zero_torque(id, exo_data), _elbow_min_max(id, exo_data), _calibr_manager(id, exo_data), _chirp(id, exo_data), _step(id, exo_data)
{
#ifdef JOINT_DEBUG
    logger::print(_is_left ? "Left " : "Right ");
