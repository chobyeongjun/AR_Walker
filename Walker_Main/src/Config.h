...
namespace loadcell_calibration
{
    ...
    // 왼쪽 무릎 로드셀 보정값
    const float left_knee_bias = 1.0f;
    const float left_knee_sensitive = 2.0f;

    // 오른쪽 무릎 로드셀 보정값
    const float right_knee_bias = 3.0f;
    const float right_knee_sensitive = 4.0f;
    
    // 왼쪽 발목 로드셀 보정값
    const float left_ankle_bias = 5.0f;
    const float left_ankle_sensitive = 6.0f;
