    ...
    if (delta_t >= (lower_bound))
    {
        ...
        while (Serial4.available()) {
            if (data->ankle_IMU_flag) {
                _imu_left_ankle.read();
                _imu_right_ankle.read();
            }
            if (data->knee_IMU_flag) {
                _imu_left_knee.read();
                _imu_right_knee.read();
            }
        }
...
void Exo::_update_imu_data()
{
    // 왼쪽 발목 IMU 데이터 업데이트
    if (data->ankle_IMU_flag) {
        this->data->left_side.ankle.imu_pitch = _imu_left_ankle.pitch;
        this->data->left_side.ankle.imu_gyro_y = _imu_left_ankle.gyro_y;
        this->data->right_side.ankle.imu_pitch = _imu_right_ankle.pitch;
        this->data->right_side.ankle.imu_gyro_y = _imu_right_ankle.gyro_y;
    }
    if (data->knee_IMU_flag) {
        this->data->left_side.knee.imu_pitch = _imu_left_knee.pitch;
        this->data->left_side.knee.imu_gyro_y = _imu_left_knee.gyro_y;
        this->data->right_side.knee.imu_pitch = _imu_right_knee.pitch;
        this->data->right_side.knee.imu_gyro_y = _imu_right_knee.gyro_y;
    }
}
