...
StatusLed::StatusLed(int r_pin, int g_pin, int b_pin)
{
    ...
    //  아두이노의 pinMode 함수는 핀의 모드를 설정하는 함수로, OUTPUT 모드로 설정하면 해당 핀을 출력으로 사용합니다.
...
void StatusLed::update(uint16_t message)
{
    if (message != _current_message) // current_message (이전에 저장된 상태) 와 다를때만 이 코드가 실행되고,
    {
        ...
        _msg_idx = status_led_defs::status_led_idx[_current_message];
        _pattern_start_timestamp = millis(); // Restart the timer 깜빡이거나 하기 위해 초기화 시키는 코드
...
void StatusLed::_set_color(int r_color, int g_color, int b_color)
{
    ...
    if (status_led_defs::has_pwm) // Using simple digital pins 우리가 PWM 핀을 사용하고 있다면 이 코드가 실행돼서 analogWrite를 사용하여 LED의 색상을 설정합니다.
    {
    ...
    else // Using simple digital pins
    {
...
void StatusLed::_pulse()
{
    if (status_led_defs::has_pwm)
    {
        ...
        // pulse는 사인파를 따라서 0 1 0 으로 변하는 주기를 보여줍니다.
