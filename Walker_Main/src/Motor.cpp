...
void _CANMotor::transaction(float torque) // transaction(torque): 모터에 토크 명령을 보내고, 모터의 현재 상태를 읽는 함수
{
    ...
    send_data(torque); // 모터에 토크 명령을 보냅니다 : 이 시스템에서는 Torque Control 사용
...
void _CANMotor::send_data(float torque) // 모터 드라이버에 명령을 내리는 함수 , 토크 컨트롤
{
#ifdef MOTOR_DEBUG
...
void _CANMotor::on_off() // on_off() 함수는 모터 드라이버의 전원을 물리적으로 켜거나 끄는 역할
{
    if (_data->estop || _error)
    {
        ...
        // 만약 비상정지(estop)가 눌렸거나 다른 에러(_error)가 발생한 상태라면, 모터의 전원 상태(_motor_data->is_on)를 강제로 false(꺼짐)로 바꿉
