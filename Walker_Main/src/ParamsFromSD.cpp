...
void print_param_error_message(uint8_t error_type)
{
    ...
    switch (error_type & ((uint8_t)config_defs::joint_id::knee | (uint8_t)config_defs::joint_id::ankle))
    {
...
// Arduino의 SD 라이브러리는 `Stream` 클래스를 상속받아 유용한 함수들을 제공합니다.
uint8_t set_controller_params(uint8_t joint_id, uint8_t controller_id, uint8_t set_num, ExoData *exo_data)
{
    ...
    File param_file;
    std::string filename;
    uint8_t header_size;       // Number of lines to skip before the parameters  // 파라미터가 시작되기 전에 건너뛸 줄 수
    uint8_t param_num_in_file; // Number of parameters to pull in  // 파일에 저장된 파라미터의 개수
    uint8_t line_to_read;      // Line to read the parameters from  // 읽을 줄의 번호
    uint8_t error_type = 0;    // Error message holder // 에러 상태를 저장할 변수 (0이면 에러 없음)

    switch (utils::get_joint_type(joint_id)) // joint_id를 기반으로 관절 타입(힙, 무릎, 발목, 팔꿈치)에 따라 분기
    {
    case (uint8_t)config_defs::joint_id::knee:
    {
