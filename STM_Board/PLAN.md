...
### Phase B — 모터 결정 후 (보류) — 전압·전류 의존 부품

| 블록 | 영향받는 항목 | 사용자 노트 |
|---|---|---|
| DC 입력 커넥터 | XT30 / XT60 / XT90 / Molex | **전류**에 의존 (전압 ↓ → 동일 전력에 전류 ↑) |
| eFuse 외부 N-MOSFET | RDS(on), V_DS 정격 (40V / 60V / 100V) | 전압·전류 둘 다 |
| 모터 출력 커넥터 | XT30 (15A) / XT60 (30A) | 전류 |
| **입력 벌크 캡 전압 정격** | **25V / 35V / 50V / 63V / 100V** | **전압 의존 — 사용자 언급 항목** |
| Common-mode choke 정격 | 15A / 30A / 60A | 전류 |
| 백업 퓨즈 정격 | 15A / 30A / 40A blade | 전류 |
| 모터 전원 폴리곤 너비 | 100mil / 200mil / 300mil | 전류 |
| **TVS 클램프 전압** | **SMCJ24A (24V) / 36A (36V) / 48A (48V) / 58A (60V)** | **전압 의존** |
| Buck 입력 캡 V 정격 | 동일 (전압) | 모터 라인 분기점 |

**INA228은 0~85V 전 범위 커버라 전압 결정 무엇이든 그대로 사용 OK.**
INA226(36V max)도 ≤36V에서 동작하지만 전압 미정인 지금 INA228이 안전. 가격·사이즈 거의 동일.

### 전압별 부품 매트릭스 (참고)

| 모터 전압 | 벌크 캡 V 정격 | TVS | MOSFET V_DS | 입력 전류 (15A 모터 ×2 동일 전력 가정) |
|---|---|---|---|---|
| 12V (예: 3S Li-ion 11.1V) | 25V | SMCJ15A | 30~40V | ↑↑ (전류 2~4배) |
| 24V (4S 14.8V or 6S 22.2V) | 35V | SMCJ24A/30A | 40V | 기본 가정 |
| 36V (8S 29.6V or 10S 36V) | 50V | SMCJ36A | 60V | ↓ |
| 48V (12S 44.4V or 14S) | 63V | SMCJ48A | 80V | ↓↓ |

> **중요:** 전압 ↓ 시 동일 전력에 **전류는 ↑**. 15A 모터 사양이 "전류 기준"이면 무관, "전력 기준"이면 저전압에서 전류 더 큰 커넥터 필요.
...
### 전압·전류 정보 받으면 좋을 것 (Phase B 들어가기 전)

- 모터 정격 전압 범위 (예: 12V / 24V / 36V / 48V)
- 배터리 셀 구성 (Li-ion 3S/4S/6S/.. 등)
- 모터 사양: 15A는 **전류** 기준인지 **전력 기준**인지
- 가속·정지 시 회생 전류 추정값 (있으면)
