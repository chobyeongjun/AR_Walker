...
## 2. MCU 확정: **STM32H723ZGT6 (LQFP144)**

| 사양 | 값 |
|---|---|
| Core | Cortex-M7 @ 550 MHz, FPU(double), DP-FPU |
| Flash | 1 MB (single bank) |
| RAM | 564 KB (DTCM 128KB + AXI/SRAM) |
| CAN-FD | FDCAN x2 (한 개만 사용) |
| USB | OTG_HS with **내장 HS PHY** (외부 PHY 불필요) ← 핵심 |
| SDMMC | SDMMC1/2 (4-bit, 50MHz) |
| Ethernet | RMII MAC 내장 (외부 PHY: LAN8742A) ← 옵션 |
| Timer | HRTIM, 다수 PWM Timer |
| 패키지 | LQFP144 (납땜 가능) — BGA 안 씀 |

**대안:** STM32H743VIT6 (LQFP100, 2MB Flash, 1MB RAM, USB HS는 외부 ULPI PHY 필요).
USB HS 내장 PHY가 있는 H723이 보드 단순화에 유리해서 H723 1순위.

> ❓ CubeMX 세팅 어떤 칩이야? H723 / H743 / H750 / 다른 거? 알려주면 핀맵 거기 맞춤.
...
### 3-1. 전원
- DC 입력: XT30 (전압은 모터 후 결정)
- 역전압: P-MOSFET ideal diode (SI4435 + Schottky)
- 보호: SMBJ TVS, Polyfuse 5A, common-mode choke
- DCDC 24V→5V: **TPS54360DDA** (60V 입력, 3.5A) — 48V까지 커버
- LDO 5V→3.3V 디지털: **TLV75533** (500mA, 저노이즈)
- LDO 5V→3.3V 아날로그: 별도 ferrite + LDO (ADC 정밀도)
- 벌크: 100µF/63V x2 + 47µF/16V + 디커플링
- 전원 LED, 전류 모니터(INA219, 옵션)
...
1. **CubeMX 세팅 칩**: H723 / H743 / H750 / 기타? (현재 H723 가정)
2. **Jetson ↔ MCU 통신 방식 (포즈 다운링크 고려해 재추천)**:
   - **(a) 고속 UART + 별도 SYNC GPIO ← 1순위 추천** : 단순, < 1ms 지연, 부품 거의 없음
   - (b) 일반 Ethernet UDP (PHY 추가) : 산업표준, 미래 확장
   - (c) USB HS : 부품 적지만 jitter
   - 둘 이상 같이 넣을 수도 있음 (UART 1순위 + USB 디버그)
3. **USB-C 보조 포트** (DFU/펌웨어 업데이트/디버그 CDC) — 항상 넣는 거 추천
4. **Ethernet PHY 풋프린트** DNF로 남길지? (보드 사이즈 ~5% 증가)
5. **ESP32 안테나**: PCB 내장(`-1`) vs 외부 IPEX(`-1U`)?
6. **E-stop**: 외부 안전 컨택터 있어? 보드는 NC 입력만 받을지?
