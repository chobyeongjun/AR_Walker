...
## 2026-04-22 v3.13 (EBIMU 데이터시트 직접 읽고 스펙 검증)

| 항목 | 교정 내용 | 근거 |
|---|---|---|
| EBIMU UART 커넥터 | ~~4pin (TX/RX/GND/5V)~~ → **3pin (TX/GND/5V)** | EBRCV24GV6 데이터시트 §2-1: 헤더 핀은 5V·GND·3V·TX. RX 핀 없음 |
| EBIMU UART baud | "~1Mbps" 추정 → **921600 bps (default)** | EBRCV24GV6 §5-1-1, sb=8 default |
| EBIMU 출력 속도 | "1kHz" 추정 → **1~1000 Hz 구성 가능, 500Hz 제어엔 sor=2** | EBRCV24GV6 §5-1-2 |
| EBIMU IMU 운용 전류 | 미상 → **30 mA** | EBIMU24GV6 §7-2 |
| EBIMU 수신기 운용 전류 | 미상 → **45 mA** | EBRCV24GV6 §6-2 |
| EBIMU 내부 자세 갱신 | 미상 → **8000 Hz** (외부 출력과 별개) | EBIMU24GV6 §8-1 |
| EBIMU 치수 | 미상 → IMU **24.6×32.0×5.3mm**, 수신기 **39.0×26.0mm** | §9 / §7 |
| EBIMU 정확도 | "대략" → Static roll/pitch **<0.1°**, yaw **<0.5°**, Dynamic RMS **<1.5°** | EBIMU24GV6 §8-1 |
| EBIMU 안테나 | 미상 → 수신기 SMA dipole 50Ω 외장 | §2-2-3 |
| EBIMU 설정 방법 | 미상 → USB-C VCP 로 `<sor2>`, `<soc2>`, `<pair>` 등 | EBRCV24GV6 §4-4 |
| BOM / Size Budget / Comms 블록 | 3pin 반영, 핀 수 정정 | 전체 |

## 2026-04-22 v3.14 (ZED 데이터시트 검증 + Jetson 이슈 명시)

| 항목 | 결정 / 발견 | 근거 |
|---|---|---|
| ZED X Mini 치수 | **93.6 × 31.8 × 36.7 mm, 151 g** (legacy 150g 근사 정확) | ZED_X_Datasheet v1.2 |
| ZED 내장 IMU | Accel ±12g + Gyro ±1000dps, 400Hz 16-bit — 카메라 Visual-Inertial 용 | §Sensor Stack |
| ZED ↔ Orin NX 연결 | **ZED Link Duo 캡처 카드 필요** (Orin NX 는 GMSL2 직결 불가) | §System Requirements |
| ZED Link Duo 전원 | 외부 12~19V 필요 (NX 사용 시) | §Accessories |
| 렌즈 옵션 | 2.2mm 광각 (FOV 110°×80°) vs 3.8mm 표준 (FOV 80°×52°) — 사용자 선택 필요 | [[Open Questions]] |
| Body Tracking | 18/34/38 keypoints 2D+3D+rotation 출력 (100Hz WVGA) | §SDK |
| Multi-camera sync | 하드웨어 sync frame-level **<100 μs** — HW SYNC GPIO 와 조합 시 sub-ms 정합도 | §Features |
| Jetson Orin NX 16GB TOPS | **100 Dense INT8 / 157 Sparse INT8** (legacy "157" 은 sparse peak 값) | NVIDIA 공식 + ZED 데이터시트 교차 |
| **Jetson PDF 레포 내 파일** | **14 KB = NVIDIA Developer 로그인 벽 에러 페이지**, 정상 아님 | 파일 크기 검증 |
| Carrier board | **미확정** 추가 — ZED Box Orin NX / Dev Kit / Seeed / Auvidea 등 | [[Open Questions]] |
