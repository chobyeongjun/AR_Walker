...
Exo::Exo(ExoData *exo_data)
    : left_ankle_imu(0),  
      ...
      status_led(logic_micro_pins::status_led_r_pin, logic_micro_pins::status_led_g_pin, logic_micro_pins::status_led_b_pin), _imu() // IMU 객체 초기화
{
    this->data = exo_data;
...
bool Exo::run()
{
    ...
    // 1주기당 시간을 us로 변환해서 10% 오차까지는 허용한다
    ...
    if (delta_t >= (lower_bound))
    {
        ...
        while (Serial4.available()) {
            if (data->ankle_IMU_flag) {
                left_ankle_imu.read();
                right_ankle_imu.read();
            }
            if (data->knee_IMU_flag) {
                left_knee_imu.read();
                right_knee_imu.read();
            }
        }

        // IMU 객체에 저장된 최신 데이터를 ExoData에 업데이트
        _update_imu_data();
...
void Exo::_update_imu_data()
{
    // 왼쪽 발목 IMU 데이터 업데이트
    if (data->ankle_IMU_flag) {
        this->data->left_side.ankle.imu_pitch = left_ankle_imu.pitch;
        this->data->left_side.ankle.imu_gyro_y = left_ankle_imu.gyro_y;
        this->data->left_side.ankle.imu_battery = left_ankle_imu.battery;
        this->data->left_side.ankle.imu_time_error_detected = left_ankle_imu.time_error_detected;
        
        this->data->right_side.ankle.imu_pitch = right_ankle_imu.pitch;
        this->data->right_side.ankle.imu_gyro_y = right_ankle_imu.gyro_y;
        this->data->right_side.ankle.imu_battery = right_ankle_imu.battery;
        this->data->right_side.ankle.imu_time_error_detected = right_ankle_imu.time_error_detected;
    }
    if (data->knee_IMU_flag) {
        this->data->left_side.knee.imu_pitch = left_knee_imu.pitch;
        this->data->left_side.knee.imu_gyro_y = left_knee_imu.gyro_y;
        this->data->left_side.knee.imu_battery = left_knee_imu.battery;
        this->data->left_side.knee.imu_time_error_detected = left_knee_imu.time_error_detected;
        
        this->data->right_side.knee.imu_pitch = right_knee_imu.pitch;
        this->data->right_side.knee.imu_gyro_y = right_knee_imu.gyro_y;
        this->data->right_side.knee.imu_battery = right_knee_imu.battery;
        this->data->right_side.knee.imu_time_error_detected = right_knee_imu.time_error_detected;
    }
}
