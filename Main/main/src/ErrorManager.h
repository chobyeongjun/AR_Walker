...
public:
    ...
    bool run(JointData *_data, ExoData *exo_data)  // ExoData 접근을 위한 매개변수 추가
    {
        // Check for errors
        for (int i_error = (NO_ERROR + 1); i_error != ERROR_CODE_LENGTH; i_error++)
        {
            // Current error code
            ErrorCodes error_code = static_cast<ErrorCodes>(i_error); // ErrorCodes 가 MOTOR_ERROR 등으로 되어있음

            // Get error type
            ErrorType *error = error_map.at(error_code);

            // Check for error
            if (error->check(_data, exo_data))
            {
                error->handle(_data, exo_data);
