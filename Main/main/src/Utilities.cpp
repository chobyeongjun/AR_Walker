...
namespace utils
{
    ...
    /*
    template <typename T>
    T rate_limit(T setpoint, T last_value, unsigned long* last_time, T rate_per_ms)
    {
    unsigned long time = millis();
    unsigned long elapsed_time = time - *last_time;

    // 경과 시간만큼 허용된 최대 변화량 계산
    T step = elapsed_time * rate_per_ms;

    *last_time = time;

    // 목표값과 (이전 값 + 최대 변화량) 중 더 작은 값을 반환하여
    // 값이 한 번에 너무 크게 변하는 것을 막음
    return std::min(setpoint, last_value + step);
    };
    */
    ...
    {
        // len 매개변수를 활용하여 안전하게 문자열 생성
        String str = "";
        for (int i = 0; i < len && arr[i] != '\0'; i++) {
            str += arr[i];
        }
        return remove_all_chars(str, rmv);
    }
