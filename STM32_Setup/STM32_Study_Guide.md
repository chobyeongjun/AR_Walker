# STM32 로봇 보드 개발 종합 학습 가이드

> **대상 칩**: STM32H743VITx (ARM Cortex-M7, LQFP-100, 480 MHz)
...
1. [STM32 아키텍처 기초](#1-stm32-아키텍처-기초)
2. [핀 시스템 완전 정복](#2-핀-시스템-완전-정복)
3. [클럭 시스템](#3-클럭-시스템)
4. [GPIO 설정 상세](#4-gpio-설정-상세)
5. [로봇에 필요한 핵심 페리페럴](#5-로봇에-필요한-핵심-페리페럴)
6. [STM32CubeMX 실전 설정 과정](#6-stm32cubemx-실전-설정-과정)
7. [AR_Walker 핀 매핑 전략](#7-ar_walker-핀-매핑-전략)
8. [보드 브링업 프로세스](#8-보드-브링업-board-bring-up-프로세스)
...
> **주의**: LQFP-100 패키지에서는 GPIOF~GPIOK는 사용 불가 (핀이 없음).
...
> **규칙**: 필요한 최소 속도를 선택한다. 속도가 높을수록 EMI(전자파 간섭)가 증가하고 소비전류가 늘어난다.
> - LED, Enable 핀 → Low
> - CAN, UART → Medium
...
> **주의**: PA6를 ADC로 사용하면 SPI1_MISO로는 사용 불가.
